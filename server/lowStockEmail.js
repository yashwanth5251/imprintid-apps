...
/**
 * Shared low-stock email sender for Azure App Service and Vercel.
 * @returns {Promise<{ok:true,to:string,itemCount:number,provider:string}>}
 */
export async function sendLowStockAlert(body = {}) {
  const items = Array.isArray(body.items) ? body.items : [];
  if (!items.length) {
    const err = new Error("No low-stock items provided.");
    err.statusCode = 400;
    throw err;
  }
